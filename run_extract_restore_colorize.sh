...
usage() {
  cat <<'EOF'
Usage: run_extract_restore_colorize.sh <input-file-or-dir> <output> [codeformer|gfpgan|realesrgan]
EOF
  exit 1
}
...
exec "$RESTORE_VENV/bin/python" -m agent_old_photo.cli extract-restore-colorize \
  "$1" \
  "$2" \
  "${3:-codeformer}" \
  --ddcolor-model "${DDCOLOR_MODEL:-ddcolor_modelscope}" \
